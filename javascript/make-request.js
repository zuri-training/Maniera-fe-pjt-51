...
for (const selectOption of optionItems) {
	selectOption.addEventListener("click", function () {
		...
		if (!this.classList.contains("selected")) {
			this.parentNode.querySelector(".option-items.selected").classList.remove("selected");
			this.classList.add("selected");
			this.closest(".display-select-input").querySelector(".option-display").textContent = this.textContent;
		}
	});
}
